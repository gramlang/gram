...
pub fn type_check_rec<'a>(
    ...
    let (elaborated_term, elaborated_type) = match &term.variant {
        ...
        Lambda(variable, implicit, domain, body) => {
            ...
            /*
            // Replace outer implicit variables in the codomain with unifiers. These will be
            // generalized below.
            while let Pi(_, true, _, codomain_codomain) = &codomain.variant {
                let unifier = Term {
                    source_range: None,
                    variant: Unifier(Rc::new(RefCell::new(None)), 0),
                };

                codomain = open(codomain_codomain, 0, &unifier, 0);

                // We could skip this check and always construct an application as per the
                // `else` branch. However, we do this to avoid creating unnecessary "junk"
                // applications.
                if let Lambda(_, true, _, body_body) = &body.variant {
                    body = open(body_body, 0, &unifier, 0);
                } else {
                    body = Term {
                        source_range: None,
                        variant: Application(Rc::new(body), Rc::new(unifier)),
                    };
                }
            }
            */
    ...
    // Collect the unifiers for generalization.
    let mut unifiers = vec![];
    let mut visited = HashSet::new();
    collect_unifiers(&elaborated_type, 0, &mut unifiers, &mut visited);
    collect_unifiers(&elaborated_term, 0, &mut unifiers, &mut visited);

    (elaborated_term, elaborated_type)

    /*
    println!("Elaborated term: {}", elaborated_term);
    println!("Elaborated type: {}", elaborated_type);

    // Generalize.
    let (generalized_term, generalized_type) = unifiers.iter().rev().enumerate().fold(
        (elaborated_term, elaborated_type),
        |acc, (i, unifier)| {
            let new_unifier = Rc::new(RefCell::new(Err(0)));

            {
                *unifier.0.borrow_mut() = Ok(Term {
                    source_range: None,
                    variant: Unifier(new_unifier.clone(), -1), // -1 to cancel out the shifts below
                });
            }

            let shifted_term = Term {
                source_range: None,
                variant: Lambda(
                    PLACEHOLDER_VARIABLE,
                    true,
                    Rc::new(type_term.clone()),
                    Rc::new(unsigned_shift(&acc.0, 0, 1)),
                ),
            };

            let shifted_type = Term {
                source_range: None,
                variant: Pi(
                    PLACEHOLDER_VARIABLE,
                    true,
                    Rc::new(type_term.clone()),
                    Rc::new(unsigned_shift(&acc.1, 0, 1)),
                ),
            };

            let generalization_variable = Term {
                source_range: None,
                variant: Variable(PLACEHOLDER_VARIABLE, unifier.1 + i),
            };

            // Resolve the unifier again to undo the shift.
            {
                *new_unifier.borrow_mut() = Ok(generalization_variable);
            }

            (shifted_term, shifted_type)
        },
    );

    println!("Generalized term: {}", generalized_term);
    println!("Generalized type: {}", generalized_type);

    (generalized_term, generalized_type)
    */
}
